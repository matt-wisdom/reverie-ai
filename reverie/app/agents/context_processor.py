...
class ContextProcessor:
        ...
        for i, page in enumerate(reader.pages):
            ...
            for line in lines:
                ...
                if 3 < len(clean_line) < 60 and (
                    clean_line.isupper() or re.match(r"^\d+(\.\d+)*\s+", clean_line)
                ):
                    # Potential heading
                    breadcrumb_str = " > ".join(current_breadcrumb + [clean_line])
                    full_text_with_meta.append(
                        {
                            "type": "heading",
                            "text": clean_line,
    ...
    def _chunk_text(self, path: Path) -> List[Dict]:
        ...
        for sent in sentences:
            ...
            if current_len > 2000:  # ~500-600 tokens
                ...
                chunks.append(
                    {"text": f"[{path.name}] {text_block}", "breadcrumb": path.name}
                )
        ...
        if current_chunk:
            chunks.append(
                {
                    "text": f"[{path.name}] {' '.join(current_chunk)}",
    ...
    def _hierarchical_split(self, text_items: List[Dict]) -> List[Dict]:
        ...
        for item in text_items:
            if item["type"] == "heading":
                if current_section:
                    final_chunks.extend(
                        self._sub_split(current_section, current_breadcrumb)
                    )
                    current_section = []
    ...
    def _sub_split(self, lines: List[str], breadcrumb: str) -> List[Dict]:
        ...
        if len(section_text) < 2400:
            return [
                {"text": f"[{breadcrumb}] {section_text}", "breadcrumb": breadcrumb}
            ]
        ...
        for p in paragraphs:
            ...
            if curr_len > 2000:
                block = "\n\n".join(curr_para)
                sub_chunks.append(
                    {"text": f"[{breadcrumb}] {block}", "breadcrumb": breadcrumb}
                )
                # Overlap
