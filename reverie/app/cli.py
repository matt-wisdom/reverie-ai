...
# --- 1. PRE-IMPORT ENVIRONMENT LOADING (CRITICAL) ---
# config.py now handles the strict .env loading hierarchy automatically on import
# But we must ensure it hasn't been imported yet.
...
def load(
    ...
    asyncio.run(
        pipeline.process_codebase(project.root_path, force=force, user_prompt=prompt)
    )
...
def summary(tag: str = typer.Argument(..., help="Project tag")):
    ...
    if not db_path.exists():
        typer.echo(
            f"Error: Project '{tag}' not found or hasn't been ingested. Run 'reverie load {tag}' first."
        )
    ...
    try:
        ...
        if res.has_next():
            ...
            if not df.empty and df.iloc[0]["p.summary"]:
                ...
                typer.echo(
                    f"Summary for project '{tag}' is empty. Try running 'reverie load {tag}' again."
                )
        else:
...
def review(
    ...
    output_dir: Optional[Path] = typer.Option(
        None, "--output-dir", "-o", help="Custom directory to save reports"
    ),
):
...
def hook_uninstall(tag: str):
    """Uninstall the reverie pre-commit hook."""
    db = get_registry_session()
    project = db.query(Project).filter(Project.tag == tag).first()
    db.close()
    if not project:
    ...
    hook_path = Path(project.root_path) / ".git" / "hooks" / "pre-commit"
    if hook_path.exists():
        # Check if it's actually our hook
        with open(hook_path, "r") as f:
            content = f.read()
        if "reverie hook run" in content:
            hook_path.unlink()
            typer.echo(f"Uninstalled hook from {hook_path}")
        else:
            typer.echo(
                "Pre-commit hook exists but does not appear to be a Reverie hook. Manual removal required."
            )
    else:
...
def hook_run(tag: str):
    ...
    # Fast Ingestion: Update KG and Vector DB for staged files only
    typer.echo(f"Updating Knowledge Graph for {len(files_to_review)} staged files...")
    from .agents.ingestion import IngestionPipeline

    pipeline = IngestionPipeline(tag=tag, skip_dirs=project.skip_dirs)
    pipeline.ladybug_client.init_schema()

    async def ingest_staged():
        for f_info in files_to_review:
            ext = f_info["path"].split(".")[-1].lower() if "." in f_info["path"] else ""
            await pipeline._process_file(f_info["path"], ext, force=True)

    asyncio.run(ingest_staged())

    typer.echo(f"Running fast pre-commit checks...")

    # Build state using user project config
    state = {
        "project_tag": tag,
