...
class ReporterAgent:
    ...
    def _save_reports(state, markdown_content, sarif_dict):
        try:
            ...
            # 1. Save to project isolated storage
            md_path = reports_dir / f"report_{timestamp}.md"
            sarif_path = reports_dir / f"results_{timestamp}.sarif"
            ...
            # 2. Save latest results to current working directory
            cwd_sarif = Path.cwd() / f"reverie_results_{timestamp}.sarif"
            with open(cwd_sarif, "w", encoding="utf-8") as f:
                f.write(json.dumps(sarif_dict, indent=2))

            logger.info(f"Latest SARIF exported to: {cwd_sarif}")
