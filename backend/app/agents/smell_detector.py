...
class SmellDetectorAgent:
    def __init__(self, tag: str, project_dir: str, project_root: str):
        ...
        def get_file(path: str):
            ...
            except Exception as e: return f"Error reading file at {target_path}: {e}"
        ...
        def get_callers(function_name: str, file_path: str):
            ...
            if res.has_next():
                return json.dumps(res.get_as_df().replace({float('nan'): None}).to_dict(orient="records"))
            return "[]"
        ...
        def emit_code_smell(title: str, severity: str, description: str, refactoring_suggestion: str, complexity_score: int, line: int):
            ...
            return json.dumps({
                "title": title, "category": "smell", "severity": severity, 
                "description": description, "remediation": refactoring_suggestion,
                "complexity_score": complexity_score, "line": line
            })

        self.tools = [get_file, get_callers, check_circular_dependencies, emit_code_smell]
    ...
    async def _call_model(self, state: AgentState):
        ...
        async with llm_limiter: response = await self.llm.ainvoke(messages)
